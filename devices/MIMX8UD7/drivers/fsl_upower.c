...
static double buck2_buck3_output_voltage_array[BUCK_OUTPUT_TABLE_SIZE] = {0};
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
int UPOWER_SetDDRRetention(soc_domain_t domain, bool enable)
{
    ...
    status = upwr_xcp_set_ddr_retention(domain, enable, UPOWER_Callback);
    if (status == 0)
    {
        UPOWER_CheckReq(UPWR_SG_EXCEPT);
    }
 ...
 * @return 0 if ok, failure otherwise.
 */
int UPOWER_ChngPmicVoltage(int rail, int voltage)
{
    int status = -1;

    status = upwr_vtm_chng_pmic_voltage(rail, voltage, UPOWER_Callback);
    if (status == 0)
    {
        UPOWER_CheckReq(UPWR_SG_VOLTM);
    }
...
int UPOWER_ReadPmicReg(uint32_t reg_addr, uint8_t *reg_val)
{
    ...
    if (status == 0)
    {
        UPOWER_CheckReqWithArgs(UPWR_SG_EXCEPT, NULL, NULL, &val, 0);
    }
 ...
 * @return 0 if ok, failure otherwise.
 */
int UPOWER_GetPmicVoltage(int rail, int *voltage)
{
    int status = -1;
    ...
    if (status == 0)
    {
        UPOWER_CheckReqWithArgs(UPWR_SG_VOLTM, NULL, NULL, voltage, 0U);
    }
