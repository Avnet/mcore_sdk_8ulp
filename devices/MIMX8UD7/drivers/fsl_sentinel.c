...
void SENTINEL_SendMessage(uint32_t *param, uint32_t paramCount)
{
    ...
    for (i = 0U; param && paramCount > 0U; paramCount--, i = (i + 1U) % SENTINEL_TR_CNT)
    {
        word = *param++;
...
void SENTINEL_ReceiveMessage(uint32_t *pParam, uint32_t *pParamCount)
{
    ...
    *pParam++ = word;
    ...
    memcpy((void *)&hdr, (void *)&word, sizeof(word));
    msgSize = hdr.size;
    assert(msgSize >= 1U);

    for (i = 1U, msgSize--; msgSize > 0U; msgSize--, i = (i + 1U) % SENTINEL_RR_CNT)
    {
...
uint32_t SENTINEL_Command(uint32_t *cmdParam, uint32_t cmdParamCount, uint32_t *pRespParam, uint32_t *pRespParamCount)
{
    ...
    memcpy((void *)&hdr, cmdParam, sizeof(hdr));
    ...
    if (pRespParam && pRespParamCount && *pRespParamCount > 0U)
    {
        ...
        memcpy((void *)&rsp, (void *)pRespParam, sizeof(rsp));
    }
    else
    {
        SENTINEL_ReceiveMessage((uint32_t *)&rsp, (uint32_t *)&rspLen);
    }
...
uint32_t SENTINEL_Ping(void)
{
    ...
    return SENTINEL_Command((uint32_t *)&hdr, sizeof(hdr) / sizeof(uint32_t), NULL, NULL);
}
...
uint32_t SENTINEL_GetFirmwareVersion(uint32_t *pVersion, uint32_t *pCommitSha)
{
    ...
    SENTINEL_Command((uint32_t *)&cmdParam, sizeof(cmdParam) / sizeof(uint32_t), (uint32_t *)&respParam,
                     &respParamCount);
...
uint32_t SENTINEL_SessionOpen(uint32_t *pSessionHandle,
                              ...
                              uint8_t pri,    /* Priority associated with the MU in the XRDC configuration */
                              uint8_t operatingMode)
{
    ...
    if (pSessionHandle != 0U)
    {
        cmdParam.mu_id         = muId;
        cmdParam.interrupt_num = intNum;
        cmdParam.tz            = tz;
        cmdParam.did           = did;
        cmdParam.priority      = pri;
...
uint32_t SENTINEL_RngOpen(uint32_t *pRngHandle, uint32_t sessionHandle, uint32_t flags)
{
    ...
    if (pRngHandle != 0U)
    {
        ...
        cmdParam.crc                = compute_msg_crc((uint32_t *)&cmdParam, sizeof(cmdParam) - sizeof(uint32_t));
    }
...
uint32_t SENTINEL_RngGetRandom(uint32_t *pRnghandle, uint32_t outAddr, uint32_t outSize)
{
    ...
    if (respParam.rsp.rsp_code.bridge_rsp_code.status == BRIDGE_SUCCESS_STATUS) /* Successful */
    {
        ...
        memcpy((void *)outAddr, share_buffer, outSize);
        assert(respParamCount == sizeof(respParam) / sizeof(uint32_t));
    }
    else
    {
...
uint32_t SENTINEL_GetSocInfo(uint16_t *pSoc_id, uint16_t *pSoc_rev)
{
    ...
    cmdParam.buf_size = sizeof(struct sentinel_get_info_data);
