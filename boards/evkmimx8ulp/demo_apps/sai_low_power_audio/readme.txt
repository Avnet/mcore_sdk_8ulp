...
2.  This case runs together with Linux and the Linux release version should be not lower than 5.15.32-2.0.0.
...
4.  Please make sure audio file (such as xxx.wav) in any filesystem that Linux can access.
    ...
    Under A core U-Boot console, execute below command to enable 8ULP board's USB device mode (USB Mass Storage) function:
      u-boot->ums 0 mmc 0
    Connect TYPE-C USB cable between 8ULP board and PC. You will see a drive show on PC to allow copy any file to borad.
...
2.  8ULP supports audio format of S16_LE, and sample rates could be 8kHz, 16kHz and 48kHz, NOT inlcuding 44.1kHz.
3.  Current demo should NOT be run with argument 'plug', otherwise LPA mode will NOT take effect:
      "aplay -Dplughw:2 -i xxx.wav -N"
4.  If want to playback with APD low power mode and specified period-size, could use command:
    ...
    Now please use "echo mem > /sys/power/state" command to make APD enter suspend mode and the playabck work normally.
    Note:
	 1. For APD into power down mode, this demo only support dual boot mode. 
	 2. make sure the A core has enough time to fill the audio buffer before going into suspend mode.
