...
#if defined(FLASH_USE_DEFAULT_LUT_TABLE)
extern const uint32_t defaultLUTOctalMode_ADESTO[CUSTOM_LUT_LENGTH];
#else
extern const uint32_t customLUTOctalMode_ADESTO[CUSTOM_LUT_LENGTH];
#endif
#endif
...
#else
extern const uint32_t customLUTOctalMode_GIGADEVICE[CUSTOM_LUT_LENGTH];
#endif
#endif
...
status_t flexspi_nor_enable_octal_mode(FLEXSPI_Type *base)
{
    ...
    if (flash_state.spiMode != SPINOR_OPI_MODE) /* When flash is not in OPI Mode */
    {
        switch (flash_state.manufacturerId) /* Manufacturer ID */
        {
            ...
            case (SNOR_MFR_ADESTO):
            {
                ...
                if ((readValue & 0x0CU) == 0x0CU)
                {
                    ...
                    FLEXSPI_UpdateLUT(base, 4 * NOR_CMD_LUT_SEQ_IDX_CONFIG, tempLUT,
                                      sizeof(tempLUT[0]) / sizeof(uint8_t));
                ...
                /* Update LUT table. */
                FLEXSPI_UpdateLUT(base, 4 * NOR_CMD_LUT_SEQ_IDX_CONFIG, tempLUT, sizeof(tempLUT[0]) / sizeof(uint8_t));
            ...
            {
                /* MXIC's octal flash. */
                uint32_t value = FLASH_ENABLE_OCTAL_CMD;
...
#if FLASH_ADESTO_DEVICE_ATXP032 || FLASH_GIGADEVICE_DEVICE_GD25LX256
...
#else
            memcpy(tempCustomLUT, customLUTOctalMode_ADESTO, sizeof(tempCustomLUT));
#endif
            /* Update LUT table into a specific mode, such as octal SDR mode or octal DDR mode based on application's
             * requirement. */
...
#if defined(EXAMPLE_FLASH_RESET_CONFIG)
            FLEXSPI_UpdateLUT(base, 4 * NOR_CMD_LUT_SEQ_IDX_READ, TempFastReadSDRLUTCommandSeq,
                              4); /* Make sure that cpu could load instruction from flash correctly(cpu will load
            ...
            if (flash_state.spiMode == SPINOR_SPI_MODE)
            {
...
#else
                memcpy(tempCustomLUT, customLUTOctalMode_GIGADEVICE, sizeof(tempCustomLUT));
#endif
                ...
                if (status == kStatus_Success)
                {
                    ...
                    if (flash_state.addrMode != true)
                    {
                        ...
                        status = flexspi_nor_exec_op(base, 0, FLASH_PORT, kFLEXSPI_Read, NOR_CMD_LUT_SEQ_IDX_CONFIG, 1,
                                                     NULL, 0);
